  ...
  public function output() {
    ...
    if( is_object( $google_json ) ) {
      ...
      if( ! empty( $is_variant ) ) {
       ...
       $font_size = (isset($font_size) && !empty($font_size))? $font_size : $defaults_value['size'];
	
		
        echo '<label class="cs-typography-variant">';
        echo '<select name="'. $this->element_name( '[size]' ) .'" data-atts="size">';
        for ($x = 10; $x <= 60; $x++) {
          echo '<option value="'. $x .'"'. $this->checked( $x, $font_size, 'selected' ) .'>'. $x .' px</option>';
        }
        echo '</select>';
        echo '</label>';
